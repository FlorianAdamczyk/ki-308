...
# Probleme mit NaN-Werten in den skalierten Daten, vermutlich durch konstante Spalten (z.B. HouseAge nach Cut-off-Entfernung).

# def Min_Max_Scaler(df, min_val=0.0, max_val=1.0):
#     if not isinstance(df, pd.DataFrame):
#         raise TypeError("Input muss ein DataFrame sein.")

#     # Features und Target sauber trennen
#     features = df.iloc[:, :-1].astype(float)
#     target = df.iloc[:, -1].astype(float)
...
#     # konstante Spalten verhindern NaN
#     ranges[ranges == 0] = 1

#     # Skalieren
#     scaled = (features - min_vals) / ranges
#     scaled = scaled * (max_val - min_val) + min_val

#     # Target wieder anhängen – sauber, ohne Object dtype
#     scaled[target.name] = target.values.astype(float)
...
# def Min_Max_Scaler_NaN(input: Union[pd.DataFrame, np.ndarray], min_val: float = 0.0, max_val: float = 1.0) -> Union[pd.DataFrame, np.ndarray]:
